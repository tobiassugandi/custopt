...
class Precond_hessian():
  ...
  def __init__(self, optimizer, model, grad_tuple, device='cuda'):
    ...
    self.state = optimizer.state["global_state"]
  ...
  def matrix_vector_product(self, v): 
    ...
    if not self.state.get("H0") == "Nys":
      v_prime = self.gamma.sqrt() * v_prime
    else:
      v_prime = _apply_nys_inv(v_prime, self.state.get("U"), self.state.get("S"), self.state.get("nys_mu"), 
                                    pow=-0.5)
    ...
    if not self.state["H0"] == "Nys":
      v_prime = self.gamma.sqrt() * (hv_prime - v_prime)
      # v_prime = self.gamma.sqrt() * v_prime
    else:
      v_prime = _apply_nys_inv((hv_prime - v_prime), self.state.get("U"), self.state.get("S"), self.state.get("nys_mu"), 
                               pow=-0.5)
